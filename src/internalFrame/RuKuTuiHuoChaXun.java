	...
	public RuKuTuiHuoChaXun() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				...
				String sql = "select * from v_rkthView where ";
				if (selectDate.isSelected()) {
					...
					list = Dao.findForList(sql + conditionStr + " "
							+ operationStr + " and rtdate>='" + sDate
							+ "' and rtdate<='" + eDate + " 23:59:59'");
				} else {
					list = Dao.findForList(sql + conditionStr + " "
							+ operationStr);
				}
		...
		showallButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list = Dao.findForList("select * from v_rkthView");
		...
		String[] tableHeads = new String[]{"入退票号", "商品编号", "商品名称", "规格", "单价",
				"数量", "金额", "供应商全称", "入退日期", "操作员", "经手人", "结算方式"};
